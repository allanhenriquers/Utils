def repository = 'ssh://git@stash.matera.com:7999/cbank/conta-corrente.git'
    ...
    [
        ...
        parameters(
            [
                ...
                string(defaultValue: 'c6-v7.11.23.f013', description: 'Branch de onde será gerada a release', name: 'branch')
            ]
        ),
        pipelineTriggers([])
    ]
...
def updateClientVersion() {
    ...
    def versionFormatted = version.substring(0, version.toLowerCase().indexOf('-c6'))

    println "versionFormatted: ${versionFormatted}"
...
def updateVersionWithMaven(String version) {
    withMaven(
                ...
                sh "mvn versions:set -DnewVersion=${version} versions:commit -q -f connector-soap-mappings/pom.xml"
                sh "mvn versions:set -DnewVersion=${version} versions:commit -q -f service/pom.xml"
                sh "mvn versions:set -DnewVersion=${version} versions:commit -q -f connector-soap/pom.xml"
